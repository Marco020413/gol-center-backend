...
<main class="max-w-7xl mx-auto px-6 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <thead class="bg-slate-800/50 text-slate-400 text-xs uppercase text-center">
        <tr>
            ...
            <th class="px-6 py-4 text-center">Acciones</th> </tr>
    </thead>
            <tbody class="divide-y divide-slate-800 text-sm">
                ...
                <tr class="hover:bg-blue-900/5 transition">
                    ...
                    <td class="px-6 py-4 text-center flex justify-center gap-2">
                        <button onclick="editarJugador('{{ $telefono }}', '{{ $j['nombre'] ?? '' }}', '{{ $j['equipo'] ?? '' }}', '{{ $j['edad'] ?? 0 }}', '{{ $j['direccion'] ?? '' }}')" class="text-blue-500 hover:text-blue-400 p-1 transition">
    ...
    <div class="space-y-6">
        <div class="bg-blue-600 p-6 rounded-xl shadow-lg text-white">
            ...
            <div class="space-y-3">
                <button onclick="abrirModal()" class="w-full bg-white text-blue-600 font-bold py-3 rounded-lg text-sm hover:shadow-xl transition">
                    Registrar Jugador
                </button>
                <button onclick="abrirModalEquipo()" class="w-full bg-blue-700 text-white font-bold py-3 rounded-lg text-sm border border-blue-400/30 hover:bg-blue-800 transition">
                    Crear Equipo
                </button>
            </div>
        </div>
...
<div id="modalJugador" class="fixed inset-0 bg-slate-950/80 backdrop-blur-sm hidden items-center justify-center z-[100] p-4">
    <div class="bg-slate-900 border border-slate-800 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden">
        <div class="p-6 border-b border-slate-800 flex justify-between items-center bg-blue-600/10">
            <h3 class="text-xl font-bold text-white">Nuevo Jugador</h3>
